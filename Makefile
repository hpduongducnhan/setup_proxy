...
configure-proxy-docker:
	...
	@# Check if http-proxy.conf exists
...
.PHONY: configure-proxy-environment
...
.PHONY: configure-proxy-bashrc

first-setup-ubuntu:
	sudo apt-get update -y
	sudo apt-get upgrade -y
	sudo make configure-proxy-docker
	sudo make configure-proxy-apt
	sudo make configure-proxy-environment
	sudo make configure-proxy-bashrc